...
import {CREATE_WATCHLIST_STOCK} from "../../actions";
...
function* createWatchlistStockAsync(action) {
    ...
    if (createStockResult.userStock) {
        yield put(push('/dashboard', {shouldRefetch: true}))
        toast.success(`Associated ${action.payload.stock.companyName} stock with your profile and watchlist.`,
            {position: toast.POSITION.BOTTOM_RIGHT});
    } else {
        yield console.log(`Errors: ${createStockResult.errors}`)
        toast.error(`Unable to associate ${action.payload.stock.companyName} stock with your profile and watchlist.`,
            {position: toast.POSITION.BOTTOM_RIGHT});
    }
}

export function* watchCreateWatchlistStockAsync() {
    yield takeEvery(CREATE_WATCHLIST_STOCK, createWatchlistStockAsync);
}
